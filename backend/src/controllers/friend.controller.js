...
export const sendFriendRequest = async (req, res) => {
  try {
    ...
    if (from === to) {
      return res.status(400).json({ message: "Bạn không thể gửi yêu cầu kết bạn với chính mình" });
    }
    ...
    res.status(201).json({ message: "Gửi yêu cầu kết bạn thành công", request });
...
export const getAllFriends = async (req, res) => {
  try {
    ...
    console.error('Lỗi khi khi lấy danh sách bạn bè:', error);
    res.status(500).json({ message: "Lỗi hệ thống" });
  }
};
...
export const getFriendRequests = async (req, res) => {
  try {
    ...
    console.error('Lỗi khi khi lấy danh sách yêu cầu kết bạn:', error);
    res.status(500).json({ message: "Lỗi hệ thống" });
  }
};