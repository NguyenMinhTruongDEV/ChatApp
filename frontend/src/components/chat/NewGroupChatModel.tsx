...
const NewGroupChatModel = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    try {
      ...
      console.error("Lỗi xảy ra khi handleSubmit trong newGroupChatModel", error);
    }
  }
        ...
        >
          ...
          <DialogFooter>
            <Button
              type="submit"
              disabled={loading}
              className="flex-1 bg-gradient-chat transition-smooth text-white hover:opacity-90"
            >
              {loading ? (
                <span>Đang tạo ...</span>
              ) : (
